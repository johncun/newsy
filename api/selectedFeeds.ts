import { FeedItem, FeedItems, FeedRequestSchema, FeedResult } from "../shared/feed-types.js"
...
export default async function handler(
  ...
  try {
    ...
    const allFeeds = await Promise.all(sources.map((feed) => parseRSSFeed(feed.url, feed.name)))
    ...
    const feedsMapped = allItems.reduce((prev, it) => {
      return { ...prev, [it.source]: [...(prev[it.source] || []), it] }
    }, {} as { [key: string]: FeedItem[] })

    const voting: VotingItem[] = sources.map(src => ({ name: src.name, n: src.votes }))
