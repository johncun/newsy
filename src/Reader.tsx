...
const normalFont = () => settings.useSans ? 'Nunito_Sans' : 'Noto_Serif'
  ...
  return props.ci.type === "image" ?
    <div class="flex flex-col items-center w-full border-0 border-slate-700/10 rounded-md _p-3">
      <div class={`${settings.fauxImage ? 'halftone' : ''}`} >
        <CachedImage class="rounded-lg" src={props.ci.url} alt={props.ci.alt} /></div>
...
const Reader = (props: { value: ReaderInput | undefined }) => {
  ...
  const parse = (): SectionItem[] | undefined => {
  ...
  const parseFromLevel = (startLevel: number): SectionItem[] | undefined => {
    const tags = props.value?.source.toUpperCase().startsWith('__toberemove__BBC') ? [`h${startLevel}`] : [1, 2, 3].slice(startLevel - 1).map(l => 'h' + l)
    console.log(tags)
  ...
  const [onContentIssue, setOnContentIssue] = createSignal(false)
  ...
  return (
    <Motion.div id="reader" ref={elRef} initial={{ x: "120vw" }} animate={{ x: ["120vw", "-15vw", 0], opacity: 1 }} transition={{ duration: 0.3, easing: "ease-in-out" }}
      ...
      <div class="absolute inset-x-0 top-12 bottom-0 overflow-y-auto overscroll-y-none">
